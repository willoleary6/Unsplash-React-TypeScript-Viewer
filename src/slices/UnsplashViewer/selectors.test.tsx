...
import { selectGalleryImageForModal } from "./selectors";
...
describe("Selector testing", () => {
    ...
    beforeEach(() => {
        const mockState: mockStoreInterface = {
            unsplash: {
                ...initialState,
                searchResults: [sampleImageResult],
                selectedGalleryImageId: sampleImageResult.id,
            },
        };
        store = mockStore(mockState);
    });
    it("Tests selector to ensure that it is working correctly ", () => {
        expect(selectGalleryImageForModal(store.getState())).toEqual(sampleImageResult);
    });
});
